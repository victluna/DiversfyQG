...
def encode_pseudo_train(outs, y, tokenizer):
    max_seq_length = 512
    input_ids = tokenizer.batch_encode_plus(outs, max_length = max_seq_length, padding = "max_length", truncation = True)#与上面的tokenizer()是一样的
    source_ids = np.array(input_ids['input_ids'], dtype = np.int32)
    source_mask = np.array(input_ids['attention_mask'], dtype = np.int32)
...
def forward_process(args, tokenizer, outs_back, model, device, iteration): #### model is the forward model, outs is the sub by backward model
    print('Loading!!!!')
        ...
        for index, ground  in enumerate(groundTruth):
            ...
            for sim, q, sub in zip(similarity[0], outs_q, subkgs):             
                if sim >= 0.7:
                    ...
                    diverse_score = diversity_changed_score(grou, q)
...
#####该函数是为backward函数测试/生成准备数据
def generate_backward_process(args, tokenizer, iteration):
    print('Loading!!!!')
    #####train_set类型为list
    train_set = []
    ##### pseundo.txt是unlabel data, 就是一些questions####
    with open(os.path.join(args.input_dir, 'pseundo.txt'), 'r') as f:
    ...
    print('shape of input_ids, attention_mask , input_ids of sparqls, choices and answers:')
    with open(os.path.join(args.output_dir, '{}.pt'.format(name)), 'wb') as f:
        for o in outputs:
            print(o.shape)
            pickle.dump(o, f)
