...
def train_forward(args, iteration):
    ...
    if iteration == 0:
        ...
        outs = generate_backward(backward_model, tokenizer, device, iteration, args)  #### backward model generate subgraphs by natural questions      
...
def train_backward(args, iteration):
    ...
    if iteration == 0:
        ...
        ##### 利用forward model 得到label data对应的y,然后传给backward_process,从而确定最终的训练backward model的data#######
        model.load_state_dict(torch.load(os.path.join(args.output_dir, 'param_backward_WQ_' + str(iteration - 1) +'.pt')) )
        backward_model = BartForConditionalGeneration.from_pretrained(args.model_name_or_path)
        backward_model.load_state_dict(torch.load(os.path.join(args.output_dir, 'param_forward_WQ_' + str(iteration - 1) +'.pt')) )
...
#####generate_forward()函数用于train backward函数时,为其准备训练数据######
...
######test_backward()函数用于初始化训练forward函数######
...
#####generate_backward()函数用于train forward函数时,为其准备训练数据######
...
def evaluate_multi_metrics(path_gold, outs, k):
    ...
    for index, ele in enumerate(outs):
        ...
        if (index+1) % k == 0:
            ...
            predict = []
    similarities = [] ###所有生成的diversity
    similarities_filter = []
    diversity_ground_scores = [] ###与ground 的diversity
    diversity_scores = [] ###两两之间的diversity
    model = SimCSE("princeton-nlp/sup-simcse-bert-base-uncased")
    filter_preds = []
